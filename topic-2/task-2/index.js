...
const arrayStripped = (array, size) => {
    ...
    let resultArr = [];
    if(array !== undefined && typeof array !== 'string' && size > 0){
        let arrInResult = Math.ceil(array.length / size);
        for (let i = 0; i < arrInResult; i++){
            resultArr[i] = array.slice((i*size), (i*size) + size);}
    }
    return resultArr;
}
